...
class CalendarMixin:
    def calendar_list_events(self, calendar_id="primary", page_size=50, page_token=None):
        """查询日程列表
    ...
    def calendar_create_event(self, calendar_id="primary", summary=None, description=None,
                               start_time=None, end_time=None, location=None, attendees=None, **kwargs):
        """创建日程
        ...
        if attendees:
            body["attendees"] = attendees if isinstance(attendees, list) else [attendees]
        body.update(kwargs)
        return self._request(
            "POST", f"/open-apis/calendar/v4/calendars/{calendar_id}/events", body=body
        )
    
    def calendar_delete_event(self, event_id, calendar_id="primary"):
        """删除日程"""
        return self._request(
            "DELETE", f"/open-apis/calendar/v4/calendars/{calendar_id}/events/{event_id}"
        )
    ...
    def calendar_update_event(self, event_id, calendar_id="primary", summary=None, description=None,
                              start_time=None, end_time=None, location=None, attendees=None, **kwargs):
        """更新日程信息
        ...
        body.update(kwargs)
        return self._request(
            "PATCH", f"/open-apis/calendar/v4/calendars/{calendar_id}/events/{event_id}", body=body
        )
    ...
    def _build_time_field(value):
        ...
        if isinstance(value, str) and ("T" in value or value.endswith("Z") or "+" in value):
            # 处理 +08:00 时区
            ts = value
            if ts.endswith("Z"):
                ts = ts[:-1] + "+00:00"
